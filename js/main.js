...
for (var i = 0; i < items.length; i++) {
    items.item(i).children[0].addEventListener('click', function(){
        nav_button.click();
    });
}
...
{
    scrolls.item(j).addEventListener("click", function(event){
        event.preventDefault();

        elem = this.getAttribute("href");
        obj  = document.getElementById(elem.substr(1, elem.length));
    ...
    if (document.documentElement.scrollTop == 0) {
        ...
        if (document.documentElement.scrollTop < destacados.offsetTop - destacados.scrollTop - width){            
            if (!mnu_inicio.parentElement.classList.contains("menu__item--select")){
                items   = document.getElementsByClassName('menu__item');
                for (var i = 0; i < items.length; i++){
                    items.item(i).classList.remove("menu__item--select");
                }
                mnu_inicio.parentElement.classList.add("menu__item--select");
            }
        }
        else if (document.documentElement.scrollTop >= destacados.offsetTop - destacados.scrollTop - width &&
        ...
        else if (document.documentElement.scrollTop >= clientes.offsetTop - clientes.scrollTop - width &&
        ...
        else if (document.documentElement.scrollTop >= nosotros.offsetTop - nosotros.scrollTop - width &&
